...
このツールの最終的なゴールは、**どんなデータセットに対しても、データアナリストが毎回直面するセットアップ作業をなくすこと**です。

現在は最も一般的なパターン（数値分布・カテゴリ集計・相関・分類）に対応しています。しかし実際のデータはもっと複雑です。欠損値はより賢く補完される必要があり、時系列データにはトレンド分析が必要で、非エンジニアはターミナルを開かずに結果を共有できる形式を求めています。

次のステップは**自動レポートパイプライン**の実現です。1コマンドを実行するだけで、インタラクティブなグラフと平易な日本語サマリーを含むHTMLレポートが生成され、チームの誰とでも共有できる状態を目指しています。
